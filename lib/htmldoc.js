 ...
 * @returns {*}
 */
var getComponents = function(srcFiles) {
...
var generateComponentPages = function(components, globalData, config, callback) {
  ...
  async.parallel([
    ...
    function(callback){
      ...
      if ( config.assets ) {
        from = path.join(cwd, config.assets.src);
        to = path.resolve(cwd, config.publish, config.assets.dest);
...
var generate = function(config, _logger) {
  ...
  components = getComponents(config.files);
